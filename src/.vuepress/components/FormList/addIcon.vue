<template>
  <div :class="['add-icon', { showIcon: exhibit }]">
    <div @click="addMenuIcon" @contextmenu="addMenuIcon" class="add-icon-mask"></div>
    <el-input
      placeholder="请输入菜单图标"
      v-model="propIcon"
      @change="handleInput"
      ...
      clearable
      @click="addMenuIcon"
      @dblclick="addMenuIcon"
      class="input-with-select"
    >
      <el-button @click.stop="addMenuIcon" slot="append" v-model:icon="propIcon"></el-button>
    </el-input>
    <!-- 菜单图标反显 -->
    <div v-if="exhibit">
      <el-button v-if="propIcon" v-model:icon="propIcon"></el-button>
      <span v-if="!propIcon" class="no-icon"> - </span>
    </div>
    {{ menuDialog }}
    <el-dialog :append-to-body="true" title="请选择菜单图标" v-model="menuDialog" width="40%">
      <div class="icon-list">
        {{ iconList }}
        <el-icon
