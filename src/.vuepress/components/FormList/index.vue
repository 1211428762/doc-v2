<template>
  <div>
    ...
    >
      ...
      >
        ...
        ></upload> -->
          ...
          <template v-if="item.type === 'slot'">
            <slot :class="{ 'form-item-disabled': exhibit }" :name="item.prop" v-bind:templateData="transfer" />
          </template>
        </el-form-item>
        <!-- 表单项,尾插槽 -->
      ...
      <slot name="append" :class="{ 'form-item-disabled': exhibit }" v-bind:templateData="transfer"> </slot>
      <div v-if="footer" class="button-wrap">
        <slot name="footer" v-bind:form="transfer"
          ><el-button v-if="!exhibit" class="form-submit" @click="submitForm(undefined, 'form')" :loading="btnLoading"
            >提交</el-button
          >
...
export default {
  ...
  methods: {
    ...
    getPlaceholder(row) {
      ...
      let placeholder
      if (row.type === 'input' || row.type === 'textarea' || row.type === 'password' || row.type === 'number') {
